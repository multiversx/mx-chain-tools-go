...
func main() {
	pathToLogsDir := "/home/beniamin/LOGS/backup/barnard"
	regex := regexp.MustCompile(`elapsed time to commit block             time \[s\] = ([\d\.]+[a-zµ]+)`)
	operation := "commit"
	computeAverageTime(pathToLogsDir, regex, operation)

	regex = regexp.MustCompile(`elapsed time to process block            time \[s\] = ([\d\.]+[a-zµ]+)`)
	operation = "process"
	computeAverageTime(pathToLogsDir, regex, operation)

	pathToLogsDir = "/home/beniamin/LOGS/backup/collapse-only-trie-leaves"
	regex = regexp.MustCompile(`elapsed time to commit block             time \[s\] = ([\d\.]+[a-zµ]+)`)
	operation = "commit"
	computeAverageTime(pathToLogsDir, regex, operation)
